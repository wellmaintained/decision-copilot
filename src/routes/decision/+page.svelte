<script lang="ts">
	...
	$: unpublishedDecisions = $decisionsForProject.filter(decision => decision.status !== 'published');
	$: publishedDecisions = $decisionsForProject.filter(decision => decision.status === 'published');
...
<div class="divider before:w-0 font-semibold">In progress</div>
<table class="table w-full bg-base-100 rounded-md shadow-sm">
	<thead>
		<tr>
			<th>Decision</th>
	...
	<tbody>
		{#each unpublishedDecisions as decision}
			<tr>
				<td>
				...
				<td>
					<button
						...
						on:click={() => goto(`/decision/${decision.id}/edit/identify`)}
					>
						<svg
...
<div class="divider before:w-0 font-semibold">Published</div>
<table class="table w-full bg-base-100 rounded-md shadow-sm">
	<thead>
		<tr>
			<th>Decision</th>
	...
	<tbody>
		{#each publishedDecisions as decision}
			<tr>
				<td>
					<h2 class="font-semibold">{decision.title}</h2>
					<p>{decision.description}</p>
				...
				<td>
					<button
						...
						on:click={() => goto(`/decision/${decision.id}`)}
					>
