 ...
 * Each step has a unique key and display label.
 */
export const DecisionWorkflowSteps = {
  IDENTIFY: { key: "identify", label: "Identify" },
  STAKEHOLDERS: { key: "stakeholders", label: "Stakeholders" },
  METHOD: { key: "method", label: "Method" },
  CHOOSE: { key: "choose", label: "Choose" },
  PUBLISH: { key: "publish", label: "Publish" },
} as const;
...
export class DecisionRelationshipTools {
  static getTargetDecisionOrganisationId(
    ...
    const pathParts = decisionRelationship.targetDecisionPath.split("/");
    const orgIndex = pathParts.indexOf("organisations");
    return orgIndex >= 0 ? pathParts[orgIndex + 1] : "";
  }

  static getTargetDecisionId(
    decisionRelationship: DecisionRelationship,
  ): string {
    const pathParts = decisionRelationship.targetDecisionPath.split("/");
...
export class Decision {
  ...
  getDocumentPath(): string {
    if (!this.organisationId) {
      throw new Error(
        "Cannot create document path: organisationId is required",
      );
    }
    return `organisations/${this.organisationId}/decisions/${this.id}`;
  }
  ...
  publish(): Decision {
    ...
    for (const relationship of blocksRelationships) {
      const targetDecisionId = DecisionRelationshipTools.getTargetDecisionId(relationship);
