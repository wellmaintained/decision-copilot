...
import { Decision, DecisionProps, DecisionRelationship, DecisionRelationshipTools } from '../Decision'
import { DecisionStateError, StakeholderError } from '../DecisionError'
...
describe('Decision Domain Model', () => {
  ...
  describe('Relationship Queries', () => {
    ...
    it('should manage supersedes relationships correctly', () => {
      ...
      expect(DecisionRelationshipTools.getTargetDecisionId(relationships[0])).toBe('decision-a');
    ...
    it('should manage blocked_by relationships correctly', () => {
      ...
      expect(DecisionRelationshipTools.getTargetDecisionId(relationships[0])).toBe('decision-b');
    ...
    it('should handle multiple relationships of the same type', () => {
      ...
      expect(relationships.map(r => DecisionRelationshipTools.getTargetDecisionId(r)).sort()).toEqual(['decision-a', 'decision-b'].sort());
    })

    it('should extract organisation ID from relationship target path', () => {
      const relationship = {
        targetDecisionPath: 'organisations/org-123/decisions/decision-x',
    ...
    it('should handle missing segments in relationship target path', () => {
      const relationship = {
        targetDecisionPath: 'some/invalid/path',
        targetDecisionTitle: 'Test Decision',
        type: 'blocks'
      } as DecisionRelationship;
