...
// STUB: These imports commented out for build bypass
  ...
  implements OrganisationsRepository
{
  ...
  async create(props: Omit<OrganisationProps, "id">): Promise<Organisation> {
    ...
    const orgDoc = await db.collection("organisations").add({
      name: props.name,
    ...
    for (const team of props.teams) {
      ...
      await Promise.all(
        team.projects.map(async (project) => {
          ...
          // });
          console.log('Project creation not implemented:', project.name);
        }),
      );
    }
